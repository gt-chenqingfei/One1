...
import com.oneone.framework.ui.glide.GlideKLModule;
...
public abstract class ProgressTarget<T, Z> extends WrappingTarget<Z> implements UIProgressListener {
    ...
    private void start() {
        GlideKLModule.expect(toUrlString(model), this);
    ...
    private void cleanup(int status) {
        ...
        onDelivered(status);
        GlideKLModule.forget(toUrlString(model));
        this.model = null;
    }
    ...
    public void onResourceReady(Z resource, GlideAnimation<? super Z> animation) {
        cleanup(STATUS_DISPLAY_SUCCESS);
        super.onResourceReady(resource, animation);
    }

    @Override
    public void onLoadFailed(Exception e, Drawable errorDrawable) {
        cleanup(STATUS_DISPLAY_FAILED);
        super.onLoadFailed(e, errorDrawable);
    }

    @Override
    public void onLoadCleared(Drawable placeholder) {
        cleanup(STATUS_DISPLAY_CANCEL);
