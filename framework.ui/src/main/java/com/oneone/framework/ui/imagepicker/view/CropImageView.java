...
public class CropImageView extends AppCompatImageView {
    ...
    private void initImage() {
        ...
        if (viewWidth != 0) {
            midPointX = viewWidth / 2;
        }
        if (viewHeight != 0) {
            midPointY = viewHeight / 2;
        }
        ...
        } else if (mStyle == Style.RECTANGLE) {
            int focusSize = Math.min(mFocusWidth, mFocusHeight);
            mFocusWidth = focusSize;
            mFocusHeight = ScreenUtil.screenHeight / 2;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            /* 头像的虚线画笔 */
            lineDashCirclePaint = new Paint();
            lineDashCirclePaint.setColor(Color.argb((int) (255 * 0.65), 255, 255, 255));
            lineDashCirclePaint.setStrokeWidth(1f);
            lineDashCirclePaint.setStyle(Paint.Style.STROKE); //空心
            ...
            canvas.restore();
             ...
             /* 头修的 虚线 */
            canvas.drawCircle(getRight() / 2, mFocusRect.top + (mFocusRect.bottom - mFocusRect.top) / 2,
                    (mFocusRect.bottom - mFocusRect.top) / 2, lineDashCirclePaint);
        }
...
//                    bitmap.recycle();
    ...
    private File createFile(File folder, String prefix, String suffix) {
        ...
        try {
            ...
            if (!nomedia.exists()) {
                boolean ret = nomedia.createNewFile();
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
